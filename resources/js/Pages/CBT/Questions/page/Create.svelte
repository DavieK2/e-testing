...
// @ts-nocheck
    ...
    import AssessmentQuestions from "../components/assessment_questions.svelte";
    import AssignedQuestions from "../components/assigned_questions.svelte";
    ...
    const downloadQuestionsExcel = () => {
        ...
        router.downloadExcelWithToken('/api/questions/download-excel', { questions: selectedQuestions }, {
            ...
            onSuccess : (response) => {
                ...
                link.href = dataURL;
                link.setAttribute('download', `${subjectTitle}_${questionBankClasses}_questions_${moment().format('Y_M_D_H_m_s')}.xlsx`);
...
<Layout >
    <SlidePanel title={ slidePanelState } showSheet={ showSlidePanel } on:onpanelstatus={ closeSheet } >
        <div class="fixed flex items-center h-16 justify-between bg-white py-4 container border-b w-[calc(100vw-15rem)] z-50">
            <div class="flex space-x-3 items-center py-4">
                <a use:inertia href={ `/assessments` }  class="text-sm font-bold text-gray-700 capitalize">Assessments</a>
                <Icons icon="chevron_right" className="h-4 w-4 fill-gray-700" />
                <a use:inertia href={ `/assessments/termly/manage/${assessmentId}` } class="text-sm font-bold text-gray-700 capitalize">{ assessmentTitle }</a>
                <Icons icon="chevron_right" className="h-4 w-4 fill-gray-700" />
                <span class="text-sm font-bold text-gray-700 capitalize">{ subjectTitle } ( { questionBankClasses } )</span>
            </div>
        </div>
        <div class="relative overflow-x-auto">
                <div class="fixed mt-16 w-[calc(100vw-15rem)] z-30 flex border-b border-gray-100 items-center h-16 bg-white">
                    <div class="px-8 w-[35rem] shrink-0 border-r border-gray-100 h-full">
                        <div class="flex items-center justify-center h-full">
                            <button on:click={ () => setQuestionType(AssessmentQuestions) } type="button" class={`text-sm font-semibold py-2 px-8 ${questionType != AssessmentQuestions ? 'text-gray-400 bg-gray-100' : 'text-gray-600 border-2 border-gray-500' } rounded-s-lg transition w-full`}>Questions</button>
                            <button on:click={ () => setQuestionType(QuestionBank) } type="button" class={`text-sm font-semibold py-2 px-8 ${questionType != QuestionBank ? 'text-gray-400 bg-gray-100' : 'text-gray-600 border-2 border-gray-500' } rounded-e-lg transition w-full`}>Question Archive</button> 
                        </div>      
                    </div>
                ...
                <div class="fixed w-[35rem] bg-white border-r border-gray-100 bottom-0 h-[calc(100vh-12rem)] overflow-y-auto">
                ...
                <div class="fixed bottom-0 h-[calc(100vh-12rem)] ml-[35rem] w-full bg-white overflow-auto">
                    <div class="relative flex flex-col h-full">
        ...
        <div slot="panel">
            ...
            { #if (slidePanelState === slidePanelStates.addSection) }
                ...
                <div class="flex flex-col space-y-6 p-3 w-[32rem]">
                    <div>
