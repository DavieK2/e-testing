<script>
    ...
    const getAssessmentSubjects = async () => {

        if( sessionStorage.getItem(`${assessmentId}_assessmentSubjects`) ){

            assessmentSubjects = JSON.parse( sessionStorage.getItem(`${assessmentId}_assessmentSubjects`) );

            selectedClassId = sessionStorage.getItem(`${assessmentId}_selectedClassId`);
        ...
        await router.getWithToken('/api/assessment-subjects/' + assessmentId, {
    ...
    const getCurrenClassSubjects = ( classId, className )=>{
        ...
        sessionStorage.setItem(`${assessmentId}_selectedClassId`, selectedClassId);
    ...
    const complete = () => {
        ...
        let subjects = { ...assessmentSubjects };

        disabled = true;

        router.postWithToken('/api/assessment/termly/complete', { assessmentId, subjects, step : "complete_assessment" }, {
    ...
    const saveToCache = () => {
        ...
        if( sessionStorage.getItem(`${assessmentId}_assessmentSubjects`) )  sessionStorage.removeItem(`${assessmentId}_assessmentSubjects`);

        sessionStorage.setItem(`${assessmentId}_assessmentSubjects`, JSON.stringify( subjects ) );
        sessionStorage.setItem(`${assessmentId}_selectedClassId`, selectedClassId);
    ...
    const clearCache = async  () => {
        ...
        sessionStorage.removeItem(`${assessmentId}_assessmentSubjects`);
        sessionStorage.removeItem(`${assessmentId}_selectedClassId`);
        hasCache = false;
