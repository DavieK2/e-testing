<script>
    ...
    const getResults = (shouldDownload) => {

        disabled = true;

        router.post('/api/assessment/t/results', { assessmentId, subjectId: '01j3z1r74j5f48qmkgyxmbtd5e', classId: selectedClass}, {
            onSuccess : (res) => {
                
                if( res.data.path ){
                    resultDownloadLink = res.data.path;
    ...
    //     subjectTitle = assessmentSubjectFilter.placeholder;

    //     router.downloadExcel('/api/assessment/t/results', { assessmentId, subjectId: assessmentSubjectFilter.value, classId: assessmentClassesFilter.value, export: true }, {
    //         onSuccess : (res) => {
    //             const temp = window.URL.createObjectURL(new Blob([res.data]));
    //             const link = document.createElement('a');
    //             link.href = temp;
    //             link.setAttribute('download', `${assessmentTitle} | ${subjectTitle}.xlsx`);
    ...
    const viewStudentResults = (studentId) => {
        router.navigateTo(`/assessments/student/result/${studentId}/${assessmentId}`);
    }
            ...
            <div class="flex items-center justify-between">
              ...
              <div class="flex flex-col border border-gray-100 rounded-lg w-full h-full bg-white mt-8 pt-4 pb-8 px-4">
                ...
                <div class="flex items-center mt-3 space-x-3 w-full">
                    <Select on:selected={ (e) => selectedSubject = e.detail.value } on:deselected={ () => selectedSubject = '' } options={ assessmentSubjects } optionsStyling="text-sm text-gray-800" placeholder="Select Course" className="w-full text-sm text-gray-400 py-2.5" />
                    <Select on:selected={ (e) => selectedClass = e.detail.value } on:deselected={ () => selectedClass = '' } options={ assessmentClasses } optionsStyling="text-sm text-gray-800" placeholder="Select Level" className="w-full text-sm text-gray-400 py-2.5" />
                    <Button { disabled } on:click={ () => getResults(true) } buttonText="Download Results" className="w-40 text-sm py-3 min-w-fit max-w-fit" />
                    <Button { disabled } on:click={ () => getResults(false) } buttonText="View Results" className="w-40 text-sm py-3 min-w-fit max-w-fit" />
                </div>
            </div>
        </div>
   </div>
...
<Modal show={ showResultsModal }>
    <div class="flex flex-col justify-center items-center h-full p-6 space-y-5">
       ...
       <div class="flex flex-col items-center space-y-6">
            <p class="font-bold text-gray-800 text-lg">Download Results</p>

            <a class="text-blue-500 text-sm" href={ resultDownloadLink }>Click to Download</a>
       </div>

       <div class="pt-3">
            <Button on:click={ () => showResultsModal = false } buttonText="Close" className="text-sm" />
       </div>
