<script>
    ...
    onMount(() => {
        ...
        router.getWithToken('/api/assessment-subjects/' + assessmentId, {
            onSuccess: (res) => {
                assessmentSubjects = res.data.flatMap((subject) => [{ placeholder: `${subject.subjectName} (${subject.subjectCode})`, value: subject.subjectId }]);
            }
        })
    ...
    const getResults = () => {
        ...
        router.post('/api/assessment/t/results', { assessmentId, subjectId: selectedSubject, classId: selectedClass}, {
            onSuccess : (res) => {
            ...
            <div class="flex items-center justify-between">
              ...
              <div class="flex flex-col border border-gray-100 rounded-lg w-full h-full bg-white mt-8 pt-4 pb-8 px-4">
                ...
                <div class="flex items-center mt-3 space-x-3 w-full">
                    ...
                    <Button { disabled } on:click={ getResults } buttonText="Get Results" className="w-40 text-sm py-3 min-w-fit max-w-fit" />
                </div>
            </div>
        </div>
   </div>
